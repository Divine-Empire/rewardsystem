...
export default function MakePaymentView() {
  ...
  const fetchPaymentData = async () => {
    ...
    try {
      ...
      const paymentData = data.map((c: any, index: number) => ({
        id: `payment_${index + 1}`,
        createdDate: c.created || "",
        code: c.code || "",
        reward: Number(c.reward) || 0,
        claimedBy: c.claimedBy || c.userDetails?.name || "",
        claimedAt: c.claimedAt || "",
        // Check both userDetails object and direct fields for phone/upiId
      ...
      alert("Error fetching payment data from local storage");
    } finally {
      setIsLoading(false);
    }
  };
    ...
    try {
      ...
      const updatePromises = selectedItems.map((code) => {
        return new Promise<void>((resolve) => {
          storageUtils.updatePaymentStatus(code, "Done");
          resolve();
        });
      });
    ...
    <div className="h-full flex flex-col gap-4 overflow-hidden">
                  ...
                  <div className="grid grid-cols-8 gap-4 text-xs font-medium text-white uppercase tracking-wider">
                    <div className="flex items-center gap-2">
                      <Checkbox
                        ...
                        className="border-white/50 data-[state=checked]:bg-white data-[state=checked]:text-sky-600"
                      />
                      Action
                    </div>
      ...
      <Dialog open={confirmDialogOpen} onOpenChange={setConfirmDialogOpen}>
        <DialogContent className="sm:max-w-md">
          ...
          <div className="bg-sky-50 rounded-xl p-4 my-4">
